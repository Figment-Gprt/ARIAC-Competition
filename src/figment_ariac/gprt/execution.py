...
# STEP 6 - Put Part at tray
            if(not jump and exec_step <= 6 and not self.exec_part.isInterupted()):
                ...
                if(self.part_plan.dest_tray_id == 1):
                    ...
                    incrementY = 0.1
                success = self.exec_part.deposit_at_tray(desired_part_pose=desired_part_pose, part_type=part_type, tray_id=tray_id,
                                                         force_check_piece=True, time_to_execute_action=1, accError=[0.009, 0.009, 0.009, 0.009, 0.015, 0.015, 0.009, 0.009, 0.1])

                if not success:
                    rospy.loginfo("[ExecutePart]: step6 failed. Reseting")
                    rospy.loginfo("[ExecutePart][STEP6] - Move tool tip up")
                    arm_actions.moveToolTipZY(
                        incrementZ=0.2, incrementY=incrementY, timeToGoal=1.2)

                    # waiting tf_manager update
                    camera_id, part_id = global_vars.tf_manager.find_part_name(
                        part_name=part_name, dad=camera_name)
                    rospy.loginfo(
                        "[ExecutePart]: DEBUG camera_id: {} ; part_id{}".format(camera_id, part_id))
                    if(len(camera_id) == 0 or len(part_id) == 0):  # part not found
                        rospy.loginfo(
                            "[ExecutePart]: step6 failed [part not found]. Reseting")
                        ...
                        if not success:
                            # TODO MOVE UP A BIT
                            rospy.logerr(
                                "[ExecutePart]: step6 failed. Could not get back to AGV")
                        self.part_plan.part.reset()
                        return False
                    # TODO any agv or a specific agv?
                    r = self.exec_part.find_part_any_agvs(
                        part_id)
                    ...
                    rospy.loginfo(
                        "[ExecutePart][STEP6] - Move Wait a bit above")
                    ...
                    if success:
                        rospy.loginfo(
                            "[ExecutePart][STEP6] - go_down_until_get_piece")
                    ...
                    if success:
                        ...
                        rospy.logerr(
                            "[ExecutePart]: step6 failed. We do not know what to do yet")
                        exec_step = +1  # STEP  - DONE
                else:
                    arm_actions.moveToolTipZY(0.3, incrementY, 1.4)
                    exec_step = +1  # STEP  - DONE
...
# TODO Check falty if the part dropped at tray as well(improve performance)
            if(not jump and exec_step <= 7 and not self.exec_part.isInterupted()):
                rospy.loginfo("\n\n[ExecutePart]: STEP 7 \n")
                # If we dont sleep we may not find the faulty one on this
                # iteration
                rospy.sleep(1)
                if(self.part_plan.dest_tray_id == 1):
                    ...
                    rospy.logerr(
                        "[ExecutePart]: step7 failed. We do not know what to do yet")
                    self.part_plan.part.reset()
                    return False
                ...
                if falty:
                    ...
                    rospy.loginfo("[ExecutePart][STEP7] - Go to discard pos")
                    arm_actions.set_arm_joint_values(list_of_joint_values=angles_discard,
                                                     time_to_execute_action=0.5)

                    arm_actions.check_arm_joint_values_published(
                        list_of_joint_values=angles_discard)

                    rospy.loginfo("[ExecutePart][STEP7] - discard pos")
...
###################       STEP 2       ###################################
            ...
            if(not jump and exec_step <= 2 and not self.exec_part.isInterupted()):  # STEP 2 - Check Any BIN
                ...
                rospy.loginfo("\n\n[ExecutePart]: STEP 2 \n")
                # step 1 - move to position in front of the piece
                success = self.exec_part.move_wait_front_part(
                    part_world_position)
                if not success:
                    rospy.loginfo("[ExecutePart]: step2 failed. Reseting")
                    self.part_plan.part.reset()
...
###################       STEP 3       ###################################
            ...
            # STEP 3 - move to position a bit above the part
            if(not jump and exec_step <= 3 and not self.exec_part.isInterupted()):
                ...
                while(attempt < max_attempt and not success):
                    ...
                    if not success:
                        attempt += 1
                        rospy.loginfo(
                            "[ExecutePart]: step3 failed. attempt#" + str(attempt))

                if not success:
                    rospy.loginfo(
                        "[ExecutePart]: step3 failed completly" + str(attempt))
                    global_vars.tf_manager.add_part_id_to_bl(part_id)
                    self.part_plan.part.reset()
                    return False
...
###################       STEP 4       ###################################
            ...
            # STEP 4 - move to position a bit above the part
            if(not jump and exec_step <= 4 and not self.exec_part.isInterupted()):
...
###################       STEP 5       ###################################
            ...
            # STEP 5 - Going down to get part and flipping if needed
...
# STEP 7 - Put Part at tray
            if(not jump and exec_step <= 7 and not self.exec_part.isInterupted()):
                ...
                if(self.part_plan.dest_tray_id == 1):
                    ...
                    solver = arm_actions.SolverType.AGV2
                    incrementY = 0.1
                success = self.exec_part.deposit_at_tray(
                    desired_part_pose=desired_part_pose, part_type=part_type, tray_id=tray_id, force_check_piece=True, time_to_execute_action=1)
                ...
                if not success:
                    ...
                    if(len(camera_id) == 0 or len(part_id) == 0):  # part not found
                        rospy.loginfo(
                            "[ExecutePart]: step7 failed [part not found]. Reseting")
                        # arm_actions.moveToolTipZY(incrementZ=0.2, incrementY=incrementY, timeToGoal=0.2)
                        success = self.exec_part.move_to_tray(tray_id=tray_id,
                                                              force_check_piece=False,
                                                              force_grp_sts=False,
                        ...
                        if not success:
                            # TODO MOVE UP A BIT
                            rospy.logerr(
                                "[ExecutePart]: step7 failed. Could not get back to AGV")
                        self.part_plan.part.reset()
                        return False
                    # rospy.sleep(1)
                    r = self.exec_part.find_part_any_agvs(
                        part_id)  # TODO any agv or a specific agv?
                    ...
                    if success:
                        ...
                        success = arm_actions.go_down_until_get_piece(world_position=part_world_position,
                                                                      ...
                                                                      part_type=part_type,
                                                                      time=3, ignore_height=False,
                                                                      distance=0.05, solver_type=solver,
                                                                      adjust=True)
...
class ExecutePart:
    ...
    def move_wait_belt(self, part_world_position,
        ...
        rospy.loginfo("[ExecutePart]: move_wait_front_part: " +
                      str(part_world_position))
        if(force_check_piece):
            grpOK = self.check_gripper(force_grp_sts)
            if (not grpOK):
                rospy.logerr(
                    "[ExecutePart]:move_wait_front_part - Gripper failed!")
                return False
    ...
    def move_towards_piece_on_belt(self, part_world_position, part_world_orientation,
        ...
        camera_pos = global_vars.tf_manager.get_transform(
            'world', "logical_camera_belt_1_frame").translation

        pos_robot = list(global_vars.current_joint_state.position)
        print(part_world_orientation)
        list_joint_values = solverBelt(
            part_world_position, part_world_orientation, part_type)
    ...
    def move_wait_front_part(self, part_world_position,
        ...
        if not success:
            ...
            rospy.logerr("[ExecutePart]: part_world_position: " +
                         str(part_world_position))
            rospy.logerr("[ExecutePart]: force_check_piece: " +
                         str(force_check_piece))
    ...
    def move_wait_above_part(self, part_world_position, part_world_orientation, part_type, solver_type=arm_actions.SolverType.BIN, a_bit_above_value=0.015, time_to_execute_action=1, accError=[0.009, 0.009, 0.009, 0.009, 0.015, 0.015, 0.009, 0.009, 0.009], adjust=False):
        rospy.loginfo("[ExecutePart]: move_wait_above_part: " +
                      str(part_world_position))
    ...
    def move_to_tray(self, tray_id, force_check_piece=True, force_grp_sts=True, time=2):
        ...
        if not success:
            ...
            if(force_check_piece):
                ...
                if (not grpOK):
                    rospy.logerr(
                        "[ExecutePart]:move_to_tray - Gripper failed!")
                    return False
                else:
                    return True
            else:
    ...
    def deposit_at_tray(self, desired_part_pose, part_type, tray_id, force_check_piece=False, force_grp_sts=True, time_to_execute_action=1, accError=[0.009, 0.009, 0.009, 0.009, 0.015, 0.015, 0.01, 0.009, 0.009]):
        ...
        part_position_at_tray, part_orientation_at_tray = calculate_order_position(
            desired_part_pose, tray_id)
        ...
        while(attempt < max_attempt and not success):
            ...
            if not success:
                attempt += 1
                rospy.loginfo(
                    "[ExecutePart]: step7 failed. attempt#" + str(attempt))
        ...
        if not success:
            ...
            if(force_check_piece):
                ...
                if (not grpOK):
                    rospy.logerr(
                        "[ExecutePart]:deposit_at_tray - Gripper failed to complete the movement holding part!")
                    return False
                else:
                    ...
                    if success:
                        ...
                        rospy.logerr(
                            "[ExecutePart]:deposit_at_tray - Gripper failed to drop part!")
                        return False
            else:
                return True
    ...
    def flip_part_bin(self, part_type):
        ...
        gripper_actions.wait_for_gripper(
            toGrip=False, max_wait=5, inc_sleep=0.01)
