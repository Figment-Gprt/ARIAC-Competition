...
def depositOnTray1(pos, rot, object_type, ignore_height=False, adjust=False):
    ...
    angle_var = (atan2(X_PIECE - X_BASE, Y_PIECE - Y_BASE) +
                 atan2(WRIST_LENGTH, Y_PIECE - Y_BASE))
    angle_shoulder_pan = 90 * degree - angle_var

    dx_base, dy_base = point_pos(
        X_BASE, Y_BASE, BASE_UPPER_DIST, 180 * degree - angle_var)
...
def depositOnTray2(pos, rot, object_type, ignore_height=False, adjust=False):
    ...
    if adjust:
        angles = [angle_elbow, -2.10, angle_shoulder_lift,
                  angle_shoulder_pan, angle_wrist, -1.57, 3.14 - angle_var + rot[2]]
    else:
        angles = [angle_elbow, -2.10, angle_shoulder_lift,
...
def computeAnglesFromSizes(a, b, c):
    ...
    rospy.loginfo("#A1,B1,C1 = " + str(A1 / degree), +
                  str(B1 / degree), + str(C1 / degree))
